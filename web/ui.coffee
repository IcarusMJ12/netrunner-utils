class DeckViewer
    ...
    makeDeckExpandedDiv: ->
        ...
        expanded_html += "<div class='control_inverted clickable' style='width:100%'; onclick=\"$(document).trigger('export_to_o8d', ['#{@side}', $('##{@side}_name')[0].value]);\">Export (o8d)</div>\n"
        expanded_html += "<div class='control_inverted clickable' style='width:100%'; onclick=\"$(document).trigger('export_to_tsv', ['#{@side}', $('##{@side}_name')[0].value]);\">Export (tsv)</div>\n"
